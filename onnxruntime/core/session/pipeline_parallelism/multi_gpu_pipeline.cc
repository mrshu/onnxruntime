...
void PipelineSession::SetupAndScheduleAllRequestsToStage0(const std::vector<OrtReq>& req_list,
  ...
  for (int req_idx = 0, num_reqs = req_list.size(); req_idx < num_reqs; ++req_idx) {
    ...
    const auto& shape = i_values[rc.second]
                            .GetTensorTypeAndShapeInfo()
                            .GetShape();
    int orig_seq_len = shape[model_ensemble_cfg.model_config_vec[0].seq_len_dim_index_in_input];
    int batch_size = shape[model_ensemble_cfg.model_config_vec[0].batch_dim_index_in_input];
...
void PipelineSession::ParseEnsembleFile(const std::string& ensemble_config_file_path, ModelEnsembleConfig& model_ensemble_cfg) {
  ...
  model_ensemble_cfg.input_ids_name = j["input_ids_name"];
  model_ensemble_cfg.position_ids_name = j["position_ids_name"];
  model_ensemble_cfg.logits_name = j["logits_name"];
  ...
  model_ensemble_cfg.num_stages = model_ensemble_cfg.model_config_vec.size();
}
